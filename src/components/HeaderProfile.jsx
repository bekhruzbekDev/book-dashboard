import { Bell, ChevronDown } from "lucide-react";
import { headerProfile } from "../constants/header";
import { useContext, useState } from "react";
import { Context } from "./Layout";
import { useNavigate } from "react-router-dom";

const HeaderProfile = () => {
  const [profileACtive, setProfileActive] = useState(false);
  const { carts, setCartModal, notification ,setNotification} = useContext(Context);
  const navigate =useNavigate()
  return (
    <div className="flex items-center gap-4 relative ">
      {profileACtive ||
        (carts.length >= 1 && (
          <span className="absolute left-16 top-[-2px] text-center px-2 rounded-full bg-red-500 text-white text-sm">
            {carts.length}
          </span>
        ))}
      <div className="relative" onClick={()=>notification ?navigate("/orders") | setNotification(false):""}>
        {notification && (
          <span className="w-3 h-3 rounded-full bg-red-500 absolute right-1 top-0"></span>
        )}

        <Bell className="cursor-pointer" />
      </div>
      <span
        className="cursor-pointer flex items-center gap-4"
        onClick={() => setProfileActive((c) => !c)}
      >
        <img
          src="data:image/png;base64,"
          alt="user-img"
          className="w-10 h-10"
        />
        <ChevronDown />
      </span>
      {profileACtive && (
        <div className=" absolute w-60  bg-white border rounded-md shadow-lg right-0 top-12 p-4">
          <ul className="flex flex-col gap-4">
            {headerProfile.map((item) => {
              return (
                <li
                  className={`flex gap-4 cursor-pointer ${
                    item.id == 3 ? "text-red-500" : ""
                  }`}
                  key={item.id}
                  onClick={() => item.id == 2 && setCartModal(true)}
                >
                  <item.icon />
                  {item.text}
                  {item.id == 2 && carts.length >= 1 && (
                    <span className=" text-center px-1 rounded-full bg-red-500 text-white text-sm">
                      {carts.length}
                    </span>
                  )}
                </li>
              );
            })}
          </ul>
        </div>
      )}
    </div>
  );
};

export default HeaderProfile;
